...
class NetworkInterface {
  private:
    ...
    void send_helper(const EthernetAddress MAC_address, const InternetDatagram &datagram);

    void queue_helper(const uint32_t ip_addr, const InternetDatagram &datagram);

    void send_ARP_request(const uint32_t ip_addr);

    void send_ARP_reply(const uint32_t ip_addr, const EthernetAddress &MAC_address);

    bool valid_frame (const EthernetFrame &frame);

    void cache_mapping(uint32_t ip_addr, EthernetAddress MAC_address);

    void clear_waitinglist(uint32_t ip_addr, EthernetAddress MAC_address);
  public:
